...
// Check if order_code is provided in the URL
if (isset($_GET['order_code'])) {
    $order_code = $_GET['order_code'];

    // Retrieve order details based on the provided order_code
    $orderDetailsQuery = "SELECT * FROM rpos_order_details WHERE order_code = ?";
    $stmt = $mysqli->prepare($orderDetailsQuery);
    $stmt->bind_param('s', $order_code);
    $stmt->execute();
    $orderDetailsResult = $stmt->get_result();

    if ($orderDetailsResult->num_rows > 0) {
        while ($orderDetails = $orderDetailsResult->fetch_assoc()) {
            // You can access order details like $orderDetails['column_name']
            $order_id = $orderDetails['order_code'];

            // Fetch the quantity for this product from rpos_order_details
            $product_id = $orderDetails['prod_id'];
            $quantity = $orderDetails['prod_qty'];

            // Fetch product details from rpos_products
            ...
            if ($productResult->num_rows > 0) {
                ...
                $_SESSION['cart'][$product_id] = array(
                    'p_id' => $product_id,
                    'p_name' => $product['prod_name'],
                    'p_price' => $product['prod_price'],
                    'p_quantity' => $quantity // Use the quantity from rpos_order_details
                );
            }
        }
    } else {
        // Order not found, handle accordingly (e.g., show an error message)
                                    ...
                                    <td>
                                        <div class="form-group">
                                            ...
                                            <input style="width: 114px;" type="number" class="form-control"
                                                   id="quantity<?php echo $prod->prod_id ?>" value="0" min="0">
        ...
        <div class="row">
            <div class="col">
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form action="make_order_update.php" method="post" id="checkoutForm">
                            ...
                            <div class='form-row'>
                                ...
                                <div class='col-md-3'>
                                    ...
                                    <input type='text' name='order_code' id='order_code'
                                           value="<?php echo  $order_id ?>" class='form-control'
                                           readonly>
                                </div>
...
<script>
    ...
    $.ajax({
        ...
        data: {
            ...
            cart_name: name,
            cart_price: price,
            cart_quantity: 1, // Always add 1 to the cart
            action: 'add'
        },
        success: function (data) {
            $('#displayCheckout').html(data);
