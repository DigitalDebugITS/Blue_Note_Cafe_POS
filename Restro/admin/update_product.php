...
if (isset($_POST['UpdateProduct'])) {
    ...
    if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])) {
        ...
        $original_quantity = $prod_quantity + $_POST['add_stock']; // Add stock to the current quantity

        // Insert Captured information to a database table
        $postQuery = "UPDATE rpos_products SET prod_code =?, prod_name =?, prod_img =?, prod_desc =?, prod_price =?, prod_quantity =?, original_quantity =? WHERE prod_id = ?";
        $postStmt = $mysqli->prepare($postQuery);
        // Bind parameters
        $rc = $postStmt->bind_param('ssssssss', $prod_code, $prod_name, $prod_img, $prod_desc, $prod_price, $original_quantity, $original_quantity, $update);
        $postStmt->execute();
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="prod_quantity" class="form-control"
                                               value="<?php echo $prod->prod_quantity; ?>"readonly>
                                    </div>
                                    <div class="col-md-6">
                                        <label>Add Stock</label>
                                        <input type="text" name="add_stock" class="form-control" value="0">
                                    </div>
                                </div>
                                <hr>
                                <div class="form-row">
